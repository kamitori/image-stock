...
class PaymentController extends BaseController {
	...
	public function confirm()
    {
        ...
        if($cart_total > 0 && $cart_count > 0)
        {
            ...
            if($obj_order)
            {
                if(Auth::user()->check())
                {
                    $this->exportOrderSendMail($obj_order->id);
                    $message = "Ordered successfully! We have sent you an email about your order's information.";
                    Session::flash('payment_message', $message);                                       
                    return Redirect::route('home');
                }
                else
                {
                    $message = 'Ordered successfully! We will contact to you as soon as possible.';
                    Session::flash('payment_message', $message);                    
                    return Redirect::route('order-cart');
                }
            }
        ...
        if($cart_total > 0 && $cart_count > 0)
        {
            ...
            if($obj_order)
            {
                ...
                if($charge)            
                {
                    ...
                    {
                        $this->exportOrderSendMail($obj_order->id);
                        $message = "Charged successfully! We have sent you an email about your order's information.";
                        Session::flash('payment_message', $message);
                        return Redirect::route('home');
                    }
                    else
                    {
                        $message = 'Charged successfully! We will contact to you as soon as possible.';
                        Session::flash('payment_message', $message);
                        return Redirect::route('payment');
                    }                    
                }
    ...
    function exportOrderSendMail($order_id)
    {
        $pdf = App::make('OrdersController')->exportToPdf($order_id, true);
        //pr($pdf);die;
        $userdata = Auth::user()->get();

        Mail::send('frontend.emails.order.success',
         array('userdata' => $userdata, 'order_id' => $order_id), function($message) use ($userdata, $pdf, $order_id) {

            $message->attachData($pdf, 'order-'.$order_id.'.pdf');
            $message->to($userdata->email, $userdata->first_name." ".$userdata->last_name)->subject('Your order from '.URL);
        });
    }  
}