...
<div id="info-editor" class="modal" style="z-index: 1000000;">
	<div class="modal-content" style="top: 12%; width: 80%; left: 10%; position: absolute; z-index: 1000000;">
		...
		<div class="modal-body form">
			<form method="POST" action="javascript:void(0)" accept-charset="UTF-8" id="image-form" class="form-horizontal form-row-seperated" enctype="multipart/form-data" novalidate>
		...
		<div class="modal-footer">
			...
			<button type="button" id="info-delete" class="btn red"><i class="fa fa-times"></i>Delete</button>
		</div>
	</div>
</div>
    ...
    blueimp.Gallery($("img", container), {
    			toggleSlideshowOnSpace: false,
              	...
              	onslide: function(index, slide){
               		...
               		var info = ['id', 'description', 'dimension', 'keywords', 'name', 'arrCategories', 'arrCollections', 'path', 'model', 'artist', 'gender', 'number_people', 'age_from', 'age_to', 'active'];
               		for( var i in info ) {
                		...
                		if(info[i] == 'path' ) {
							...
							if( data == 1 ) {								
								$('#info-editor #active2 option[value="1"]').prop('selected', true);
							} else {
								$('#info-editor #active2 option[value="0"]').prop('selected', true);
							}
...
$('#info-editor #info-delete').click(function(){
	var id = $('#info-editor [name=id]').val();
	console.log(id)
	if( id ) {
		bootbox.confirm('Do you sure you want to delete this image?', function(result) {
			if(result) {
				$.ajax({
					...
					success: function(result) {
						if( result.status == 'ok' ) {
							...
							// container.masonry();
							$('#info-editor [data-dismiss="modal"]').trigger('click');
							$('#blueimp-gallery .close').trigger('click');
