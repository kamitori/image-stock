...
<div id="banner-background-add-div" class="modal fade" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title">Add Banner Background</h4>
            </div>
            <div class="modal-body form">
                <form id="banner-background-add-form" action="javascript:void(0)" method="POST" class="form-horizontal form-row-seperated">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="banner-background-add-submit" class="btn btn-primary"><i class="fa fa-check"></i>Save change</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-md-12">
        <div class="portlet">
            <div class="portlet-title">
                <div class="caption">
                    <i class="icon-credit-card"></i>{{ 'Banner Background Listing' }}
                </div>
                <div class="actions">
                    <a href="#banner-background-add-div" data-toggle="modal" class="btn default yellow-stripe">
                        <i class="fa fa-plus"></i>
                        <span class="hidden-480">{{ 'New Banner Background' }}</span>
                    </a>
                </div>
            </div>
            <div class="portlet-body">
                <div class="table-container">
                    <table class="table table-striped table-hover table-bordered" id="list-banner-background">
                        <thead>
...
#image-div .modal-dialog, #banner-background-add-div .modal-dialog{
    width: 100%;
    margin: 0;
}
</style>
...
$("#banner-background-add-submit").click(function(){
    $("#content-message","#banner-background-add-form").html("You have some form errors. Please check below.");
    $("#banner-background-add-form").submit();
});
$("#banner-background-add-form").validate({
    errorElement: 'span',
    ...
    invalidHandler: function (event, validator) {
        $(".alert-danger","#banner-background-add-form").show();
        Metronic.scrollTo($(".alert-danger","#banner-background-add-form"), -200);
    },
    ...
    submitHandler: function (form) {
        var form = $("#banner-background-add-form");
        ...
        var ajaxConfig  = {
                            url : "{{ URL }}/admin/banners-backgrounds/update-banner-background",
                            type: "POST",
                            success: function(result) {
                                if( result.status == "error" ) {
                                    ...
                                    $("#banner-background-add-div").modal("hide");
                                    $("[name=name]", form).val("");
									$("[name=type]", form).val("banner");
        ...
        var files = $("[name=image]", "#banner-background-add-form")[0].files;
        var process = false;
...
$(".editable-submit", "#image-div").click(function(){
    ...
    var ajaxConfig  = {
                        ...
                        success: function(result) {
                            if( result.status == "ok" ) {
                                $("img[data-id="+ id +"]", "#list-banner-background").attr("src", result.path);
...
var columnDefs = [
		{
            ...
            "data" : function(row, type, val, meta) {
                return '<a href="javascript:void(0)" class="xeditable-input" data-escape="true" data-name="name" data-type="textarea" data-pk="'+row[1]+'" data-url="{{ URL.'/admin/banners-backgrounds/update-banner-background' }}" data-title="Name">'+row[2]+'</a>';
            }
        },
        {
            "targets": 3,
        ...
        },		
        {
            ...
            "name"  : "order_no",
            "data" : function(row, type, val, meta) {
                return '<a href="javascript:void(0)" class="xeditable-input" data-escape="true" data-name="order_no" data-type="number" data-pk="'+row[1]+'" data-url="{{ URL.'/admin/banners-backgrounds/update-banner-background' }}" data-title="Order">'+ row[5] +'</a>';
            }
        },
        {
            "targets": 6,
            ...
            "data" : function(row, type, val, meta) {
                ...
                return '<a href="javascript:void(0)" class="xeditable-select" data-escape="true" data-name="active" data-type="select" data-value="'+row[6]+'" data-pk="'+row[1]+'" data-url="{{ URL.'/admin/banners-backgrounds/update-banner-background' }}" data-title="Active">'+ html +'</a>';
            }
        },
    ];
listRecord({
    url: "{{ URL.'/admin/banners-backgrounds/list-banner-background' }}",
    delete_url : "{{ URL.'/admin/banners-backgrounds/delete-banner-background' }}",
    table_id: "#list-banner-background",
    columnDefs: columnDefs,
    fnDrawCallback: function(){
        $(".xeditable-input","#list-banner-background").editable();
        $(".xeditable-textarea","#list-banner-background").editable();
        $(".xeditable-select","#list-banner-background").editable({
