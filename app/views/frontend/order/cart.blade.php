				...
				<div class="col-12 col-lg-12" style="text-align:left">
						<table class="table table-striped">
                            ...
                            @foreach($cart_content as $row)
							  <tr>
								...
								<td>
                                	@if(Route::currentRouteName() == 'order-cart')
                                	<input type="number" id="qty_{{ $row->rowid }}" value="{{ $row->qty }}" class="form-control" style="width:75px" onblur="updateQty('{{ $row->rowid }}', this.value, event)" onclick="this.focus()" min="1" />
					  ...
					  <hr>
						<dl class="dl-horizontal pull-right">
						  ...
						  <dd>$0.00</dd> 
						  <dt>Total:</dt>
						  <dd><b>$<span id="total">{{ number_format($cart_total, 2) }}</span></b></dd>
						</dl>
