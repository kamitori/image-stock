...
Route::post('/admin/login',['before' => 'guest.admin', function () {
    ...
    if( in_array($admin['email'], ['admin', 'anvydigital0519@gmail.com'])
...
Route::group(array('prefix'=>'account','before' => 'guest'), function() {
    ...
    Route::group(array('before' => 'csrf'), function() {
        ...
        Route::post('/forgot', 'AccountController@forgot');
...
Route::get('/similar-images/{id_image}/{image_name}.html',              ['as' => 'similar-images', 'uses' => 'ISImagesController@viewAllSimilarImages']);
Route::get('/similar-images/{id_image}',              ['as' => 'similar-images1', 'uses' => 'ISImagesController@viewAllSimilarImages']);
Route::get('/user-reference/{user_id}/{first_name}.html',              ['as' => 'user-reference', 'uses' => 'ISImagesController@viewAllSameArtistImages']);
Route::get('/user-gallery/{user_id}/{first_name}.html',              ['as' => 'user-gallery', 'uses' => 'UserController@loadGallery']);
Route::get('/user-collection/{user_id}/{first_name}.html',              ['as' => 'user-collection', 'uses' => 'UserController@loadLightboxes']);
Route::get('/user-about/{user_id}/{first_name}.html',              ['as' => 'user-about', 'uses' => 'UserController@about']);
...
Route::group(array('prefix'=>'account','before' => 'auth'), function() {
    ...
    Route::get('/profile',              ['as' => 'profile-user', 'uses' => 'AccountController@show']);
    Route::post('/change-avatar',              ['uses' => 'AccountController@changeAvatar']);
    Route::get('/load-inform',              ['uses' => 'AccountController@loadInform']);
...
Route::group(array('before' => 'auth'), function()
{
    ...
    Route::get('/design-{image_id}/{name}.html', ['as' => 'design-image', function($image_id, $name){
        return app()->make('DesignsController')
                        ->callAction('index', [$image_id]);
    }])
    ->where([
        'image_id'  => '\d+',
...
Route::post('/design/get-images', 'DesignsController@getImages');
Route::post('/design/put-image-store', 'DesignsController@putImageSession');
Route::get('/design/clear-image-store/{image_id}/{name}', 'DesignsController@clearImageSession')
        ->where([
        'image_id'  => '\d+',
        'name'      => '^[-a-z0-9]*',
    ]);

Route::post('/design/put-background',     ['uses' => 'DesignsController@saveBackground']);
Route::get('/design/remove-background', 'DesignsController@removeBackground');
Route::post('/design/analyze-image',       ['uses' => 'DesignsController@analyzeImage']);
